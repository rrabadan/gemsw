...
std::string 
GEMFileIterator::Entry::get_data_path() const {
...
GEMFileIterator::EorEntry
GEMFileIterator::EorEntry::load_eor(const std::string& run_path,
                                    const std::string& filename) {
...
GEMFileIterator::GEMFileIterator(edm::ParameterSet const& pset)
  : state_(EOR) 
{
...
unsigned int GEMFileIterator::lastEntryFound() {
  if (!entrySeen_.empty())
    return entrySeen_.rbegin()->first;
  return 1;
}

void GEMFileIterator::advanceEntry(unsigned int entry, std::string reason) {
...
unsigned GEMFileIterator::mtimeHash() const {
...
void GEMFileIterator::collect(bool ignoreTimers) {
...
void GEMFileIterator::delay() {
  usleep(delayMillis_ * 1000);
}

void GEMFileIterator::logFileAction(const std::string& msg, const std::string& fileName) const {
  edm::LogAbsolute("fileAction") << std::setprecision(0) << edm::TimeOfDay() << "  " << msg << fileName;
  edm::FlushMessageLog();
}

void GEMFileIterator::logEntryState(const Entry& entry, const std::string& msg) {
...
void GEMFileIterator::fillDescription(edm::ParameterSetDescription& desc) {
